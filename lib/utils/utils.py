...
from lib.utils.loss import CrossEntropy, OhemCrossEntropy, CrossEntropyST, antiCrossEntropyST
...
def loss_calc(pred, label, config):
    ...
    criterion = CrossEntropy(config, ignore_label=255).cuda()
...
def loss_calc_ohem(pred, label, config):
    ...
    criterion = OhemCrossEntropy(config, ignore_label= 255, #config.TRAIN.IGNORE_LABEL,
...
def testval(config, test_dataset, testloader, model, model_D=None, sv_dir='', sv_pred=True):
    model.eval()
    ...
    with torch.no_grad():
        for index, batch in enumerate(tqdm(testloader)):
            ...
            if sv_pred:
                ...
                sv_path = '/home/hnu/桌面/pre_pots/HARC'
...
def validate(config, testloader, model):
    ...
    for i in range(nums):
        ...
        IoU_array = (tp / np.maximum(1.0, pos + res - tp))
        IoU_array = IoU_array#[:-1]
        mean_IoU = IoU_array[:-1].mean() # ISPRS
...
def validate_isaid(config, testloader, model):
    model.eval()
    nums = config.MODEL.NUM_OUTPUTS
    confusion_matrix = np.zeros((config.DATASET.NUM_CLASSES, config.DATASET.NUM_CLASSES, nums))
    with torch.no_grad():
    ...
    return mean_IoU, IoU_array, PA, f1, f1_array
