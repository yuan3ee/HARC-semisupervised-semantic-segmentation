...
class postdam_augstrong(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        label = Image.fromarray(np.array(Image.open(datafiles['label'])))
        name = datafiles["name"]
        ...
        return normalize(image_w), normalize(image_s1), normalize(image_s2)
    '''
    def __getitem__(self, index):
        datafiles = self.files[index]
        ...
        if 'test' in self.list_path:
            ...
            return image.copy(), _, np.array(size), name, index
        ...
        # print('image', image)
        # image_weak = image.transpose([2,0,1])
        # print('image_Weak', image_weak.shape)
        image_weak, label = self.gen_sample(image, label, self.scale, self.is_mirror)

        # image_weak_ = (image_weak- np.min(image_weak)) / (np.max(image_weak) - np.min(image_weak))
        # img_nrm = (img - np.min(img)) / (np.max(img) - np.min(img))
        # im = Image.fromarray(np.uint8(255 * img_nrm))
        # image_aug = image_weak

        image_weak_ = image_weak.transpose([1,2,0])
        image_aug = Image.fromarray(np.uint8(image_weak_))
        image_aug = self.strong_aug(image_aug)
        image_aug = np.array(image_aug)
        image_aug = image_aug.astype(np.float32)
        image_aug = self.input_transform(image_aug)
        image_aug = image_aug.transpose([2, 0, 1])

        # image_aug /= self.std
        # print('imageweak', image_weak.shape, image_weak.dtype)
        ...
        # print('!!!!!!!!!!')
        return image_weak.copy(), image_aug.copy(), label.copy(), np.array(size), name, index
    '''
...
class loveda(data.Dataset):
    ...
    def save_pred(self, preds, sv_path, name, D_out_z=None):
        ...
        for i in range(preds.shape[0]):
            ...
            pred.save(os.path.join(sv_path, name[i]+'.png'))
            # gt2color(pred, os.path.join(sv_path, name[i]+'.png'))
            # save_img.save(os.path.join(sv_path, name[i]+'.png'))
    ...
    def __getitem__(self, index):
        ...
        if 'test' in self.list_path:
            ...
            return image.copy(), _, np.array(size), name, index
        label = cv2.imread(datafiles["label"], cv2.IMREAD_GRAYSCALE)
        # print('1', np.unique(label))
        label = cv2.imread(datafiles["label"], cv2.IMREAD_GRAYSCALE) - 1
        # print('label', np.unique(label))
        # print('i1111', image.shape)
        image, label = self.gen_sample(image, label, self.scale, self.is_mirror)
...
class loveda_augstrong_cutmix(data.Dataset):
    def __init__(self, root, list_path, downsample_rate=1, num_samples=None, label_flag='remain', base_size=1024, max_iters=None, crop_size=(512, 512), mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225], scale=True, mirror=True, ignore_label=-1, scale_factor=16):
        self.root = root
        self.list_path = list_path
        self.crop_size = crop_size
    ...
    def __getitem__(self, index):
        datafiles = self.files[index]
        image = Image.open(datafiles['img']).convert('RGB')
        label = Image.fromarray(np.array(Image.open(datafiles['label'])) - 1)
        name = datafiles["name"]
        ...
        return normalize(image_w), normalize(image_s1), normalize(image_s2), cutmix_box1, cutmix_box2
    '''
    def __getitem__(self, index):
        datafiles = self.files[index]
        image = cv2.imread(datafiles["img"], cv2.IMREAD_COLOR)
