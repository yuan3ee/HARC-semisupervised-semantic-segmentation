...
class vaihingen(data.Dataset):
    ...
    def save_pred(self, preds, sv_path, name, D_out_z=None):
        ...
        for i in range(preds.shape[0]):
            ...
            pred3DPath = sv_path + '/S-out/{}.png'.format(name[i])
            pred3D = self.readPred(pred)
            cv2.imwrite(os.path.join(sv_path, name[i]+'.png'), pred3D)
    ...
    def readPred(self, pred):
        pred3D = np.zeros((3, 512, 512))
        for i in range(512):
            for j in range(512):
                if pred[i, j] == 0:
                    pred3D[:, i, j] = [255, 255, 255]
                elif pred[i, j] == 1:
                    pred3D[:, i, j] = [0, 0, 255]
                elif pred[i, j] == 2:
                    pred3D[:, i, j] = [0, 255, 255]
                elif pred[i, j] == 3:
                    pred3D[:, i, j] = [0, 255, 0]
                elif pred[i, j] == 4:
                    pred3D[:, i, j] = [255, 255, 0]
                elif pred[i, j] == 5:
                    pred3D[:, i, j] = [255, 0, 0]
        pred3D = pred3D.transpose((1, 2, 0))
        pred3D = pred3D[..., ::-1]
        return pred3D
