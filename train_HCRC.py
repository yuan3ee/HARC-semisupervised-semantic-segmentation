...
def get_arguments():
    ...
    parser.add_argument('--cfg',
                        default='/home/hnu/LY/semi_ly_1008/experiments/vaihingen/SEMI_contast_cutmix.yaml',
                        # default = './experiments/postdam/SEMI_contast_cutmix.yaml',
...
def main():
    ...
    if config.DATASET.LABELED_RATIO is not None:
        ...
        train_dataset = dst(config.DATASET.ROOT,
                           ...
                           '/home/hnu/LY/dataset/vaihingen/train_1_4.lst',
    ...
    # criterion_u_mix = nn.CrossEntropyLoss(reduction='none').cuda()
    ...
    w = 0.1
    RC_memory = region_memory(num_class=config.DATASET.NUM_CLASSES)
    for i_epoch in range(1, epoch+1):
        RC_memory = region_memory(num_class=config.DATASET.NUM_CLASSES)

        if i_epoch <= start_unsuper:
            # bar_supervised = tqdm(trainloader_gt)
            ...
            for iter, batch in tqdm(enumerate(trainloader_gt)):
                ...
                keys, vals = computer_region_center(pred_list[-2], pred_list[-1], num_class=config.DATASET.NUM_CLASSES)
                for class_i in range(config.DATASET.NUM_CLASSES):
                    RC_memory = update_region_memory(RC_memory, keys, vals, class_i)
                if i_epoch == 1:
                    loss_lab_contrast = region_contrast_loss(config, keys, vals, RC_memory)
                else:
                    loss_lab_contrast = region_contrast_loss(config, keys, vals, RC_memory_use)
                if config.LOSS.USE_OHEM:
                    loss_ce = loss_calc_ohem(pred_list_ce, labels, config)
                    loss_ce = loss_ce.mean()
                else:
                    loss_ce = loss_calc(pred_list_ce, labels, config)
                # loss_rc = RC(pred_list_new[2], pred_list_new[3])
                # loss_sup = loss_ce + contrast_weight * loss_rc
                loss_sup = loss_ce + contrast_weight * loss_lab_contrast
                # loss_sup = loss_ce
                loss_sup.backward()
                optimizer.step()
                del keys, vals
            ...
            # logger.info(
            #     'epoch = {}, iter = {}/{}, lr:{:.6f}, loss_sup = {:.3f}, lambdaST = {:.3f}'.format(
            #         i_epoch, i_iter, trainSteps, lr, loss_sup_value.average(), lambdaST,
            #         ))
        else:
            print('semi-supervised', i_epoch)

            iter = 0
            for bar_supervised, bar_unsupervised, bar_unsupervised_mix in zip(trainloader_gt, trainloader_unlabel, trainloader_unlabel):
                iter += 1
                i_iter = i_epoch * (len(train_dataset) / batch_size) + iter
                lambdaST = adjustLambdaST(i_iter, beginSTStep, againSTStep, config.TRAIN.ST_AF)
                optimizer.zero_grad()
                ...
                images_weak_mix, images_aug1_mix, images_aug2_mix, _, _ = bar_unsupervised_mix
                ...
                images_weak_mix, images_aug1_mix, images_aug2_mix = Variable(images_weak_mix).cuda(), Variable(images_aug1_mix).cuda(), Variable(images_aug2_mix).cuda()
                ...
                with torch.no_grad():
                    ...
                    pred_list_T_mix = model_teacher(images_weak_mix)
                    pesudo_label_mix = pred_list_T_mix[1]
                    pesudo_label_mix = pesudo_label_mix.softmax(dim=1)
                    pesudo_label_mix = torch.argmax(pesudo_label_mix, dim=1)


                # pred_list_new = model(images_aug1)
                # pred_list = pred_list_new[:2]
                images_aug2[cutmix2.unsqueeze(1).expand(images_aug2.shape) == 1] = \
                    images_aug2_mix[cutmix2.unsqueeze(1).expand(images_aug2.shape) == 1]
                pesudo_label[cutmix2 == 1] = pesudo_label_mix[cutmix2 == 1]

                num_lb, num_ulb, num_ulb_cutmix = images.shape[0], images_aug1.shape[0], images_aug2.shape[0]
                aux, pred, fea_con, pred_con = model(torch.cat((images, images_aug1, images_aug2)))
                pred_list_lb = [aux[:num_lb], pred[:num_lb], fea_con[:num_lb], pred_con[:num_lb]]
                pred_list_ulb = [aux[num_lb : num_lb+num_ulb], pred[num_lb : num_lb+num_ulb], fea_con[num_lb : num_lb+num_ulb], pred_con[num_lb : num_lb+num_ulb]]
                pred_list_new_mix0 = [pred[num_lb+num_ulb:], fea_con[num_lb+num_ulb:], pred_con[num_lb+num_ulb:]]

                # keys, vals = RC_memory(pred_list_lb[-2], pred_list_lb[-1], batch_size=batch_size, mode_for='supervised')
                # loss_lab_contrast = region_contrast_loss(config, keys, vals, RC_memory)
                keys, vals = computer_region_center(pred_list_lb[-2], pred_list_lb[-1], num_class=config.DATASET.NUM_CLASSES)
                for class_i in range(config.DATASET.NUM_CLASSES):

                    # print('keys', class_i,keys)
                    RC_memory = update_region_memory(RC_memory, keys, vals, class_i)
                if i_epoch == 1:
                    loss_lab_contrast = region_contrast_loss(config, keys, vals, RC_memory)
                else:
                ...
                loss_sup = (loss_ce + contrast_weight * loss_lab_contrast) / 2.0
                # loss_sup_value.update(loss_sup.item())

                # keys_weak, vals_weak = RC_memory(pred_list_ulb[-2], pred_list_ulb[-1], batch_size=batch_size, mode_for='unsupervised')
                # loss_weak_contrast = region_contrast_loss(config, keys_weak, vals_weak, RC_memory)
                keys_weak, vals_weak = computer_region_center(pred_list_ulb[-2], pred_list_ulb[-1],
                                                    num_class=config.DATASET.NUM_CLASSES)
                if i_epoch == 1:
                    loss_weak_contrast = region_contrast_loss(config, keys_weak, vals_weak, RC_memory)
                else:
                    loss_weak_contrast = region_contrast_loss(config, keys_weak, vals_weak, RC_memory_use)
                if config.LOSS.USE_OHEM:
                ...
                loss_unsup_weak = (loss_unsup_weak + contrast_weight * loss_weak_contrast) / 2.0

                # keys_mix, vals_mix = RC_memory(pred_list_new_mix0[-2], pred_list_new_mix0[-1], batch_size=batch_size,
                #                                  mode_for='unsupervised')
                # loss_mix_contrast = region_contrast_loss(config, keys_mix, vals_mix, RC_memory)
                keys_mix, vals_mix = computer_region_center(pred_list_new_mix0[-2], pred_list_new_mix0[-1],
                                                              num_class=config.DATASET.NUM_CLASSES)
                if i_epoch == 1:
                    loss_mix_contrast = region_contrast_loss(config, keys_mix, vals_mix, RC_memory)
                else:
                    loss_mix_contrast = region_contrast_loss(config, keys_mix, vals_mix, RC_memory_use)

                loss_unsup_mix = criterion_u_mix(pred_list_new_mix0[0], pesudo_label)
                loss_unsup_mix = (loss_unsup_mix + contrast_weight * loss_mix_contrast) / 2.0

                # loss_unsup_weak_value.update(loss_unsup_weak.item())
                # loss_unsup_mix_value.update(loss_unsup_mix.item())

                loss = (loss_sup + (1-w) * loss_unsup_weak + w * loss_unsup_mix) / 2.0
                # loss.backward(torch.ones_like(loss))
                loss.backward()
                optimizer.step()
        #
        #         # del bar_unsupervised, images, labels, pred_list_new, pred_list
        #
        #     logger.info(
        #         'epoch = {}, iter = {}/{}, lr:{:.6f}, loss_sup = {:.3f}, lambdaST = {:.3f}, loss_unsuper = {:.3f}'.format(
        #             i_epoch, i_iter, trainSteps, lr, loss_sup_value.average(), lambdaST, loss_unsup_value.average(),
        #         ))
        #
        # 更新teacher
        # update teacher model with EMA
                ...
                with torch.no_grad():
                    ...
                    for buffer_train, buffer_eval in zip(model.buffers(), model_teacher.buffers()):
                        buffer_eval.data = buffer_eval.data * ema_decay + buffer_train.data * (1 - ema_decay)
                        # buffer_eval.data = buffer_train.data
        RC_memory_use = RC_memory
        with open('RC_memory_1_8_potsdam.pkl', 'wb') as f:
            pickle.dump(RC_memory, f)
        # del RC_memory
        # test
        ...
        if mean_IoU > best_mIoU:
            best_mIoU = mean_IoU
            # torch.save(model.module.state_dict(), os.path.join(rootOutputDir, 'best_mIoU.pth'))
            # # print('rootOutputDir', rootOutputDir)
            # torch.save(RC.state_dict(), os.path.join(rootOutputDir, 'RC_best_mIoU.pth'))
            torch.save({
